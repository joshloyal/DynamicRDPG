...
for metric in ['auc', 'aupr']:
    ...
    for l, density in enumerate(densities):
        for k, nu in enumerate(nus):
            ...
            g = sns.lineplot(x='n_nodes', y=f'rdpg_{metric}', label='GB-DASE (RW(2) Prior)',
            ...
            ax[k, l].set_xticks([node_sizes[0]] + node_sizes[2:])
            if l != 0:
                ax[k, l].set_ylabel(' ')

            data_subset_rw = data_rw1.query(f"density == '{density}'").query(f"nu == '{nu}'")
            sns.lineplot(x='n_nodes', y=f'rdpg_rw1_{metric}', label='GB-DASE (RW(1) Prior)',
            ...
            sns.lineplot(x='n_nodes', y=f'ase_{metric}', label='ASE',
                    data = data_subset, marker='o', ax=ax[k, l],
                    errorbar='sd', linestyle=':', lw=lw, markersize=ms)

            sns.lineplot(x='n_nodes', y=f'omni_{metric}', label='OMNI',
                    data = data_subset, marker='o', ax=ax[k, l],
                    errorbar='sd', linestyle='-.', lw=lw, markersize=ms)
            
            sns.lineplot(x='n_nodes', y=f'true_{metric}', label='Oracle',
                    data = data_subset, marker='o', ax=ax[k, l], color='black',
                    errorbar='sd', linestyle='-.', lw=lw, markersize=ms)

            ax[k, l].legend(ncol=2, fontsize=16)

    fig.savefig(f'recovery_nodes_{metric}.pdf', dpi=300, bbox_inches='tight')
