...
k_steps = 6
metric = 'aupr'
metric_name = {'auc': 'AUC', 'aupr': 'AUPR'}

for metric in ['auc', 'aupr']:
    fig, ax = plt.subplots(figsize=(20, 24), nrows=3, ncols=2, sharex=True, sharey=True)
    for l, n_time_steps in enumerate([100, 150]):
        for k, density in enumerate(densities):
            res_dir = f'output/d{density}/rw2_n{n_nodes}_T{n_time_steps}_d{density}'
            data = pd.read_csv(join(res_dir, 'results.csv'))
            data['rdpg_rw2_kstep_mse_0'] = data[f'rdpg_rw2_{metric}_0']
            res_dir = f'output/d{density}/rw1_n{n_nodes}_T{n_time_steps}_d{density}'
            data_rw1 = pd.read_csv(join(res_dir, 'results.csv'))
            data_rw1['rdpg_rw2_kstep_mse_0'] = data_rw1[f'rdpg_rw2_{metric}_0']
            
            
            forecast_data = []
            for kstep in range(1, k_steps):
                var_name = f'kstep_{metric}_{kstep}'
                row = {
                    'rdpg_rw2': np.sqrt(data[f'rdpg_rw2_{var_name}']).mean(),
                    'rdpg_rw2_std': np.std(np.sqrt(data[f'rdpg_rw2_{var_name}'])),
                    'rdpg_rw1': np.sqrt(data_rw1[f'rdpg_rw2_{var_name}']).mean(),
                    'rdpg_rw1_std': np.std(np.sqrt(data_rw1[f'rdpg_rw2_{var_name}'])),
                    ...
                    'omni': np.sqrt(data_rw1[f'omni_{var_name}']).mean(),
                    'omni_std': np.std(np.sqrt(data_rw1[f'omni_{var_name}'])),
                    'mase': np.sqrt(data_rw1[f'mase_{var_name}']).mean(),
                    'mase_std': np.std(np.sqrt(data_rw1[f'mase_{var_name}'])),
                    'k_step': kstep
                }
            ...
            if metric == 'aupr':
                ax[k, l].set_ylim(0.35, 1.0)
            else:
                ax[k, l].set_ylim(0.7, 1.0)

    fig.savefig(f'forecasts_ncr_n{n_nodes}_{metric}.pdf', dpi=300, bbox_inches='tight')
